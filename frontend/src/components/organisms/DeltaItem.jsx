  ...
  return (
    <Card className={hasUnsavedChanges ? 'border-orange-300 bg-orange-50/50' : ''}>
      <CardContent className="pt-6 space-y-4">
        {/* Top Row: Delta Name Input, Complete Checkbox, Date, Delete Button */}
        ...
        <div>
          <Label className="text-sm font-medium text-gray-700">
            {name || 'Delta item'} notes
          </Label>
        </div>

        {/* Three Column Row: Runbook, Migrated, Owner */}
        <div className="grid grid-cols-3 gap-4">
          <div>
            <Label htmlFor={`runbook-${delta.id}`} className="text-sm">Runbook URL</Label>
            <Input
              id={`runbook-${delta.id}`}
              type="url"
          ...
          <div>
            <Label htmlFor={`migrated-${delta.id}`} className="text-sm">Migrated</Label>
            <Select
              value={fields.migrated === null ? '__UNSET__' : fields.migrated.toString()}
              onValueChange={(val) => handleFieldChange('migrated', val === '__UNSET__' ? null : val === 'true')}
            >
              <SelectTrigger id={`migrated-${delta.id}`} className="mt-1">
          ...
          <div>
            <Label htmlFor={`owner-${delta.id}`} className="text-sm">Owner</Label>
            <Select
              value={fields.owner || '__UNSET__'}
              onValueChange={(val) => handleFieldChange('owner', val === '__UNSET__' ? null : val)}
            >
              <SelectTrigger id={`owner-${delta.id}`} className="mt-1">
                <SelectValue placeholder="Select owner" />
              </SelectTrigger>
              <SelectContent>
                <SelectItem value="__UNSET__">Not Assigned</SelectItem>
